...
void PrintNodeInOrder(p_node_t node, int type)
{
	int value = -1;
	if (node == NULL)
		return;
	PrintNodeInOrder(node->lchild, type);
	if (type == 0)
		value = node->key;
	else if (type == 1 && node->lchild != NULL)
			value = node->lchild->key;
	else if(type == 2 && node->rchild != NULL)
			value = node->rchild->key;
	printf("%4d", value);
	PrintNodeInOrder(node->rchild, type);
}

void PrintList(p_bst_t bst)
{
	printf("node  ");
	PrintNodeInOrder(bst->root, 0);
	printf("\nleft  ");
	PrintNodeInOrder(bst->root, 1);
	printf("\nright ");
	PrintNodeInOrder(bst->root, 2);
	printf("\n");
}
